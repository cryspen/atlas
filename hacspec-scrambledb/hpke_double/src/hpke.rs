...
fn hpke_mode_label(mode: Mode) -> ByteSeq {
    match mode {
        ...
        Mode::mode_double => byte_seq!(0x04u8),
    }
}
...
pub fn HpkeSeal(
    ...
    let (enc, (key, nonce, _, _)) = match mode {
        ...
        Mode::mode_double => unimplemented!(),
    }?;
...
// === Double Encryption Extension ===
pub struct DoubleContext {
    pub sender: Context,
    pub relay: Context,
}
pub type DoubleContextResult = Result<DoubleContext, HpkeError>;
pub struct DoubleCiphertext {
    pub is_level_two: bool,
    pub ciphertext: ByteSeq,
}
...
pub fn SetupDoubleR(
    config: HPKEConfig,
    encS: &EncodedHpkePublicKey,
    encX: &EncodedHpkePublicKey,
    skR: &HpkePrivateKey,
    info: &Info,
) -> DoubleContextResult {
    let shared_secret_S = Decap(kem(config), encS, skR)?;
    let shared_secret_X = Decap(kem(config), encX, skR)?;
    let key_schedule_S = KeySchedule(
        config,
        &shared_secret_S,
        info,
        &default_psk(),
    ...
    let key_schedule_X = KeySchedule(
        config,
        &shared_secret_X,
        info,
        &default_psk(),
    ...
    DoubleContextResult::Ok(DoubleContext {
        sender: key_schedule_S,
        relay: key_schedule_X,
    })
}
...
///     return ct
/// ```
pub fn DoubleContextS_Seal(
    aead_id: AEAD,
    context: Context,
    aad: &ByteSeq,
    pt: &ByteSeq,
) -> Result<(DoubleCiphertext, Context), HpkeError> {
    let (key, base_nonce, seq, exp) = context;
    let nonce = ComputeNonce(aead_id, &base_nonce, seq);
    let ct = AeadSeal(aead_id, &key, &nonce, aad, pt)?;
    let seq = IncrementSeq(aead_id, seq)?;
    Result::<(DoubleCiphertext, Context), HpkeError>::Ok((
        DoubleCiphertext {
            is_level_two: false,
            ciphertext: ct,
        },
        (key, base_nonce, seq, exp),
    ))
}
...
pub fn DoubleContextX_ReSeal(
    ...
    if ct.is_level_two {
        return Err(HpkeError::InvalidParameters);
    }
    let (key, base_nonce, seq, exp) = context;
    let nonce = ComputeNonce(aead_id, &base_nonce, seq);
    let cct = AeadSeal(aead_id, &key, &nonce, aad, &ct.ciphertext)?;
    let seq = IncrementSeq(aead_id, seq)?;
    Result::<(DoubleCiphertext, Context), HpkeError>::Ok((
        DoubleCiphertext {
            is_level_two: true,
...
///     return pt
/// ```
pub fn DoubleContextR_Open(
    aead_id: AEAD,
    context: DoubleContext,
    aad: &ByteSeq,
    cct: &DoubleCiphertext,
) -> Result<(ByteSeq, DoubleContext), HpkeError> {
    if !cct.is_level_two {
        return Err(HpkeError::InvalidParameters);
    }
    let (keyX, base_nonceX, seqX, expX) = context.relay;
    let nonceX = ComputeNonce(aead_id, &base_nonceX, seqX);
    let ct = AeadOpen(aead_id, &keyX, &nonceX, aad, &cct.ciphertext)?;
    let seqX = IncrementSeq(aead_id, seqX)?;

    let (keyS, base_nonceS, seqS, expS) = context.sender;
    let nonceS = ComputeNonce(aead_id, &base_nonceS, seqS);
    let pt = AeadOpen(aead_id, &keyS, &nonceS, aad, &ct)?;
    let seqS = IncrementSeq(aead_id, seqS)?;
    Result::<(ByteSeq, DoubleContext), HpkeError>::Ok((
        pt,
        DoubleContext {
            sender: (keyS, base_nonceS, seqS, expS),
            relay: (keyX, base_nonceX, seqX, expX),
        },
    ))
}
